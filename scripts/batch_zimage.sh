...
prompts = {
    ...
    "guqianfan_happy": "1boy, male, ancient Chinese warrior, dark outfit, genuine happy smile, rare carefree, eyes bright, half body, warm sunlight, anime style, high quality, solo",
    ...
    "guqianfan_blush": "1boy, male, ancient Chinese warrior, dark outfit, slight blush on tanned skin, awkwardly looking away, flustered, half body, warm pink, anime style, high quality, solo",
    "guqianfan_cold": "1boy, male, ancient Chinese warrior, dark outfit, unreadable, dead eyes, emotionless, cold killer gaze, half body, cool blue, anime style, high quality, solo",
    ...
    "gongsunlan_surprised": "1girl, female, mature ancient doctor, white robe, eyes wide, hand covering mouth, unexpected news, half body, anime style, high quality, solo",
    "gongsunlan_blush": "1girl, female, mature ancient doctor, white robe, slight mature blush, shy eyes, looking down soft smile, half body, warm pink, anime style, high quality, solo",
...
def build_prompt(text, char_id, expr, seed):
    return {
        ...
        "20": {"class_type": "SaveImage", "inputs": {"images": ["2", 0], "filename_prefix": f"z_{char_id}_{expr}"}},
    }
...
def submit_and_wait(text, char_id, expr, timeout=600):
    ...
    req = urllib.request.Request(f"{COMFY_API}", data=body, headers={"Content-Type": "application/json"})
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            result = json.loads(resp.read())
    except urllib.error.HTTPError as e:
        return f"HTTP {e.code}: {e.read().decode()[:200]}", None
                ...
                if s == "error":
                    ...
                    for m in data["status"].get("messages", []):
                        if m[0] == "execution_error":
                            msg = m[1].get("exception_message", "unknown")
