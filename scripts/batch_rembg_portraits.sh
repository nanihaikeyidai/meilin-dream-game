#!/bin/bash
# ===== 批量立绘rembg去背景 =====
# 对 assets/portraits/{templateId}/{charId}/{expr}.png 中所有 RGB 文件执行 rembg
...
# 扫描所有非RGBA的PNG（template/char/expr 三级目录）
for tpl_dir in "$ASSETS_DIR"/*/; do
    tpl_id=$(basename "$tpl_dir")
    for char_dir in "$tpl_dir"*/; do
...
")
        if [ "$ct" = "6" ]; then
            echo "⏭ $tpl_id/$char_id/$expr (already RGBA)" >> "$LOGFILE"
            continue
        fi
        
        echo "🎨 $tpl_id/$char_id/$expr (RGB -> RGBA)..." >> "$LOGFILE"
        
        # cp到ComfyUI input
        cp "$png" "/mnt/f/ComfyUI_V6.0/ComfyUI-WorkFisher-V2/ComfyUI/input/rembg_${tpl_id}_${char_id}_${expr}.png"
        
        # 调用ComfyUI rembg模板
        "$PYTHON" -X utf8 /home/daixiaowei/.hermes/profiles/july/skills/mlops/comfyui-local/templates/comfy_rembg.py \
            "rembg_${tpl_id}_${char_id}_${expr}.png" "rembg_${tpl_id}_${char_id}_${expr}" >> "$LOGFILE" 2>&1
        
        # 找输出文件
        found=$(ls "$COMFY_OUTPUT"/rembg_${tpl_id}_${char_id}_${expr}_*.png 2>/dev/null | head -1)
        if [ -n "$found" ]; then
            cp "$found" "$png"
            echo "✅ $tpl_id/$char_id/$expr replaced with RGBA" >> "$LOGFILE"
        else
            echo "❌ $tpl_id/$char_id/$expr rembg output not found" >> "$LOGFILE"
        fi
    done
    done
done
